...
function Label({ children, color, size, tracking, style: ex }) {
  return (
    <span style={{
...
function SayeNav2({ current, navigate }) {
...
function Btn2({ children, variant = 'primary', onClick, full, disabled, style: ex }) {
...
function Input2({ label, placeholder, type = 'text', value, onChange, textarea, rows }) {
...
function RoleCard2({ role, desc, selected, onClick }) {
  const [h, setH] = React.useState(false);
...
function DiscoverCard2({ name, role, discipline, location, tags, onClick }) {
  const [h, setH] = React.useState(false);
  const r = ROLE_CONFIG[role] || ROLE_CONFIG.Artist;
...
function ArchiveCard2({ type, title, content, author, authorRole, date, link, hint, span, tall }) {
  const [h, setH] = React.useState(false);
...
Object.assign(window, {
  T, ROLE_CONFIG,
  Label, RuleLine, SectionMark, RoleBadge,
  SayeNav2, Btn2, Input2, Chip2,
  RoleCard2, DiscoverCard2, ArchiveCard2,
});
